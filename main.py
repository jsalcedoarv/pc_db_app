...
def index():
    ...
    if not isinstance(parts, dict):
        print("Error: Parts is not a dictionary.")
        return "Error: Parts is not a dictionary."
    
    parts_list = []
    for key, value in parts.items():
        # Ensure each value is a dictionary
        if not isinstance(value, dict):
            print(f"Error: Value for key {key} is not a dictionary. Value: {value}")
            continue
        parts_list.append({'id': key, **value})
...
def generate_unique_id():
    ...
    for part in parts.values():
        if 'gpzid' in part and part['gpzid'].startswith("GPZ") and part['gpzid'][3:].isdigit():
            num = int(part['gpzid'][3:])
            if num > max_id:
                max_id = num
...
def add_part():
    ...
    new_part_key = new_part_ref.key  # Get the unique key generated by Firebase

    new_part_ref.set({
        'id': new_part_key,  # Set the id to the Firebase database reference key
        'gpzid': unique_id,  # Unique ID is stored inside the data, not as the key
        'name': name,
        'part_type': part_type,
...
def build_pc():
    ...
    if request.method == 'POST':
        customer_details = {
        'name': request.form.get('name', '').strip() or 'Pending...',
        'address': request.form.get('address', '').strip() or 'Pending...',
        'ebay_name': request.form.get('ebay_name', '').strip() or 'Pending...',
        'build_date': request.form.get('build_date', '').strip() or 'N/A',
        'tracking_number': request.form.get('tracking_number', '').strip() or 'Pending...',
        'sold': float(request.form.get('sold_for') or 0),
        'pcid': request.form.get('pcid', '').strip() or 'Pending...'
    }
            ...
            if part_type == 'RAM':  # Handle RAM separately
                ...
                if ram1_id and ram1_id in available_parts:
                    ...
                    selected_parts['RAM 1'] = {
                        'id': ram1_id,
                        'gpzid': ram1_details.get('gpzid','Unknown'),
                ...
                if ram2_id and ram2_id in available_parts:
                    ...
                    selected_parts['RAM 2'] = {
                        'id': ram2_id,
                        'gpzid': ram2_details.get('gpzid','Unknown'),
                ...
                if part_id and part_id in available_parts:
                    ...
                    selected_parts[part_type] = {
                        'id': part_id,
                        'gpzid': part_details.get('gpzid','Unknown'),
...
@app.route('/replace_part/<build_id>/<part_type>', methods=['GET', 'POST'])
def replace_part(build_id, part_type):
    build_ref = db.reference(f'pc_builds/{build_id}')
    build = build_ref.get() or {}
    selected_parts = build.get('selected_parts', {})

    inventory_ref = db.reference('pc_parts')
    inventory = inventory_ref.get() or {}

    if request.method == 'POST':
        new_part_id = request.form.get('new_part_id')
        old_part_id = request.form.get('old_part_id')

        # Ensure the new part exists in inventory and the old part exists in the build
        if new_part_id in inventory and part_type in selected_parts:
            # Get old part details
            old_part = selected_parts[part_type]
            
            # Get new part details
            new_part = inventory[new_part_id]

            # Return old part to inventory
            inventory_ref.child(old_part['id']).set(old_part)

            # Update selected_parts with new part
            selected_parts[part_type] = {
                'id': new_part_id,
                'gpzid': new_part.get('gpzid', 'Unknown'),
                'name': new_part.get('name', 'Unknown'),
                'part_type': new_part.get('part_type', 'Unknown'),
                'serial_number': new_part.get('serial_number', 'N/A'),
                'price': new_part.get('price', 0),
                'audit_date': new_part.get('audit_date', 'N/A'),
                'part_note': new_part.get('part_note', 'N/A')
            }

            # Remove new part from inventory
            inventory_ref.child(new_part_id).delete()

            # Save the updated build back to Firebase
            build_ref.update({'selected_parts': selected_parts})

            return redirect(url_for('build_details', build_id=build_id))

    # Prepare data for the template
    available_parts = [(key, part) for key, part in inventory.items() if part['part_type'] == part_type]
    old_part = selected_parts.get(part_type, {})

    return render_template('replace_part.html', build_id=build_id, part_type=part_type, available_parts=available_parts, old_part=old_part)
